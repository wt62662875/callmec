...
@interface TripOneController()<UITableViewDataSource,UITableViewDelegate,CellTargetDelegate,CallBackDelegate>
...
- (void) fetchData
{
    ...
    [OrderModel fetchOrderListById:[GlobalData sharedInstance].user.userInfo.ids types:@"1" page:1 succes:^(NSArray *result, int pageCount, int recordCount) {
        NSLog(@"%@",result);
...
- (void) fetchMoreData
{
    ...
    [OrderModel fetchOrderListById:[GlobalData sharedInstance].user.userInfo.ids types:@"1" page:_page succes:^(NSArray *result, int pageCount, int recordCount) {
        NSLog(@"%@",result);
        if (result) {
            for (NSDictionary *dict in result) {
                OrderModel *model = [[OrderModel alloc] initWithDictionary:dict error:nil];
