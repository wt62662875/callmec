...
+ (void)saveSearchResult:(CMLocation*)location
{
    ...
    if (result) {
        if (location) {
            ...
            {
                [result insertObject:[CMLocation toDictionary:location] atIndex:0];
                if ([result count]>20) {
                    [result removeObjectAtIndex:20];
                }
                [SandBoxHelper saveSearchList:result];
            }else{
                for (int i = 0 ; i<result.count; i++) {
                    if ([[result[i] objectForKey:@"name"] isEqualToString:[[CMLocation toDictionary:location] objectForKey:@"name"]]) {
...
+ (BOOL) hasExits:(NSMutableDictionary*)dict inList:(NSMutableArray*)list
{
    ...
    for (int i = 0; i<list.count; i++) {
        if ([[list[i] objectForKey:@"name"] isEqualToString:[dict objectForKey:@"name"]]) {
            result = YES;
            break;
        }
    }
