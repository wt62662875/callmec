...
- (void) initView
{
    ...
    UILabel *payMoneyLbael = [[UILabel alloc]init];
    payMoneyLbael.text = [NSString stringWithFormat:@"￥%@",[[NSUserDefaults standardUserDefaults] objectForKey:@"PayMoney"]];
    payMoneyLbael.textColor = RGBHex(g_blue);
    payMoneyLbael.font = [UIFont systemFontOfSize:15];
    [_container addSubview:payMoneyLbael];
    [payMoneyLbael mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(_container);
        make.left.equalTo(_container).offset(80);
