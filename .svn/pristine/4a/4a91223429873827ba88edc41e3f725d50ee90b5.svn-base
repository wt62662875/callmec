...
@interface TripThreeController()<UITableViewDataSource,UITableViewDelegate,CallBackDelegate,CellTargetDelegate>
...
- (void) fetchData
{
    ...
    [OrderModel fetchOrderListById:[GlobalData sharedInstance].user.userInfo.ids types:@"4" page:1 succes:^(NSArray *result, int pageCount, int recordCount) {
        NSLog(@"%@",result);
...
- (void) fetchMoreData
{
    ...
    [OrderModel fetchOrderListById:[GlobalData sharedInstance].user.userInfo.ids types:@"4" page:_page succes:^(NSArray *result, int pageCount, int recordCount) {
        NSLog(@"%@",result);
        if (result) {
            for (NSDictionary *dict in result) {
                OrderModel *model = [[OrderModel alloc] initWithDictionary:dict error:nil];
...
- (void) cellTarget:(id)sender
{
    if ([sender isKindOfClass:[OrderModel class]]) {
        ...
        {
            WaitingGoodsCarController *order_ctrl = [[WaitingGoodsCarController alloc] init];
